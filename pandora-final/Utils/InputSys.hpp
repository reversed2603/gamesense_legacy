...
public:
   ...
   bool IsInBox( Vector2D box_pos, Vector2D box_size );
   void RegisterHotkey( int vk, std::function< void( void ) > f );
   void RemoveHotkey( int vk );
   
   Vector2D GetMousePosition( );
   float GetScrollMouse( );
   void SetScrollMouse( float scroll );
